...
/*   Updated: 2022/02/11 16:13:27 by min-kang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_philo	*create_philo(t_thm thm, t_starve starve, t_arg args)
{
	t_philo	*philo;
	int		i;
	...
	while (++i < args.p_nb)
	{
		...
		philo[i].last_meal = &starve.last_meal[i];
		if (args.e_nb)
			philo[i].e_nb = args.e_nb;
		philo[i].done = thm.done;
		philo[i].f_l = &thm.m[i];
		philo[i].f_r = &thm.m[(i + 1) % args.p_nb];
		philo[i].start = starve.start;
	}
	return (philo);
}

void	launch_philo(t_philo *philo, t_thm thm, t_starve starve, t_arg args)
{
	int			i;
	pthread_t	th_die;
	int			*enough;
	...
	pthread_create(&th_die, NULL, &die, &starve);
...
void	philosophers(t_arg args)
{
	...
	starve = die_initialise(thm, args);
	philo = create_philo(thm, starve, args);
	launch_philo(philo, thm, starve, args);
	free_n_destroy(thm, philo, starve);
}
